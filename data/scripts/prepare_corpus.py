...
- Clean + sentence-split ALL languages found in raw_dir
- Create train/dev/test splits
- Build char vocabulary (all languages)
- Train BPE tokenizers (1k & 2k) on all languages
...
def sentence_split(text: str, use_nltk: bool = False, lang_hint: str | None = None) -> List[str]:
    if use_nltk and nltk_available:
        ...
        try:
            sents = sent_tokenize(text, language=LANG_HINTS.get(lang_hint, 'english'))
        except Exception:
...
def process_language(lang: str, raw_dir: Path, clean_dir: Path, seed: int,
                     min_chars: int, max_chars: int) -> Dict[str, int]:
    random.seed(seed)
...
def main():
    ...
    # Detect languages automatically from raw_dir
    all_langs = sorted([p.name for p in args.raw_dir.iterdir() if p.is_dir()])
    print(f"[INFO] Found languages: {all_langs}")

    stats = {}
    for lang in all_langs:
        s = process_language(lang, args.raw_dir, args.clean_dir,
                             args.seed, args.min_chars, args.max_chars)
        stats[lang] = s
